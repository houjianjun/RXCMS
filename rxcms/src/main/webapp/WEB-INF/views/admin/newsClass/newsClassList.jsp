...
<div class="context">
		...
		<table cellpadding="0" cellspacing="0" border="0"
			...
			<tbody>
				<c:forEach items="${list}" var="newsClass">
					<tr>
						<td align="center">
							<div class="checker" id="uniform-undefined">
								<span> <input type="checkbox" name="chks" class="chks"
									checked="checked" value="${ newsClass.id}" style="opacity: 0;">
								</span>
							</div>
						</td>
						<td>${newsClass.name}</td>
						<td>${newsClass.desc}</td>
						<td><input id="btnEdit" onclick="btnEdit(${ newsClass.id})" type="button" class="edit" value="编辑"></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
...
<script type="text/javascript">
	$("#btnAdd").on("click", function() {
		$(location).attr('href', '/admin/newsClass/addOrEdit?id=');
	});
	//删除事件
